...
export default function BudgetOverview() {
    ...
    return (
      <Card className="p-8 text-center">
        <div className="max-w-md mx-auto">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Create Your First Budget</h3>
  ...
  const daysInMonth = new Date(currentBudget.endDate.getFullYear(), currentBudget.endDate.getMonth() + 1, 0).getDate()
  const daysElapsed = Math.floor((Date.now() - currentBudget.startDate.getTime()) / (1000 * 60 * 60 * 24))
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{currentBudget.name}</h2>
            <p className="text-gray-600">
              {format(currentBudget.startDate, 'MMM dd')} - {format(currentBudget.endDate, 'MMM dd, yyyy')}
            </p>
          </div>
        ...
        <div className="mb-6">
          ...
          {isOverBudget && (
            <div className="flex items-center space-x-1 mt-2 text-red-600">
              ...
              <span className="text-sm">Over budget by {formatCurrency(summary.totalSpent - summary.totalBudgeted)}</span>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-green-600">{summary.categoriesOnTrack}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-red-600">{summary.categoriesOverBudget}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">Expected vs actual spending</span>
              <div className="flex items-center space-x-2">
                {progressPercentage > expectedSpendingRate ? (
                  <>
                    <TrendingUp className="h-4 w-4 text-orange-600" />
                    <span className="font-medium text-orange-600">
                      +{formatPercentage(progressPercentage - expectedSpendingRate)} above pace
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="font-medium text-green-600">
                      {formatPercentage(expectedSpendingRate - progressPercentage)} under pace
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`font-medium ${
                ...
                progressPercentage > 75 ? 'text-orange-600' :
                'text-green-600'
              }`}>
                {progressPercentage > 90 ? 'High' :
                 progressPercentage > 75 ? 'Medium' : 'Low'}
              </span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-gray-900">
                {formatCurrency(daysElapsed > 0 ? summary.totalSpent / daysElapsed : 0)}
              </span>
            </div>
          </div>
        </Card>
