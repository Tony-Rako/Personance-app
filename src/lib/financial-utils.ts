...
// Calculate net worth
export const calculateNetWorth = (totalAssets: number, totalLiabilities: number) => {
...
export const calculateFIProgress = (currentNetWorth: number, targetAmount: number) => {
  if (targetAmount === 0) return 0
  return Math.min((currentNetWorth / targetAmount) * 100, 100)
}

// Calculate monthly cash flow
export const calculateCashFlow = (monthlyIncome: number, monthlyExpenses: number) => {
  return monthlyIncome - monthlyExpenses
}

// Calculate debt-to-income ratio
export const calculateDebtToIncomeRatio = (monthlyDebtPayments: number, monthlyIncome: number) => {
  if (monthlyIncome === 0) return 0
  return (monthlyDebtPayments / monthlyIncome) * 100
}

// Calculate emergency fund months
export const calculateEmergencyFundMonths = (emergencyFund: number, monthlyExpenses: number) => {
...
export const calculateInvestmentReturn = (currentValue: number, initialValue: number) => {
...
export const calculateSavingsRate = (monthlyIncome: number, monthlyExpenses: number) => {
  if (monthlyIncome === 0) return 0
...
export const calculateAllocationPercentage = (assetValue: number, totalPortfolioValue: number) => {
...
export const calculateTimeToGoal = (
  ...
  return Math.log(1 + (remaining * monthlyRate) / monthlyContribution) / Math.log(1 + monthlyRate)
}
...
export const calculateRetirementReadiness = (
  ...
  const futureValue = currentSavings * Math.pow(1 + monthlyReturn, monthsToRetirement) +
